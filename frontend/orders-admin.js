...
function costText(c){return c.estimated_usd===null?'未知':'≈ $'+Number(c.estimated_usd).toFixed(4);}
...
async function load(){
 ...
 td=node('td',costText(row.cost));td.append(node('div',`已知 ${row.cost.known_attempts} / ${row.cost.attempts} 次尝试`,'small'));tr.append(td);
...
async function detail(id){
 ...
 for(const [label,value] of [['商户订单号',d.order_no],['任务状态',taskNames[d.status]],['订单价格',d.price_cny?'¥'+d.price_cny+' · '+d.price_scope:'未知'],['支付状态',payNames[d.payment.status]],['支付宝查询金额',d.payment.amount?'¥'+d.payment.amount:'未知'],['最近核验',dateText(d.payment.checked_at)],['创建时间',dateText(d.created_at)],['模型',d.model||'不适用'],['Token 成本',costText(d.cost)],['已知费用记录',`${d.cost.known_attempts} / ${d.cost.attempts} 次尝试`],['本次输入 Token',d.cost.prompt_tokens??'未知'],['本次输出 Token',d.cost.completion_tokens??'未知']]){const el=node('div');el.append(node('dt',label),node('dd',value));grid.append(el);}body.append(grid,node('p',d.cost.note,'notice'));
