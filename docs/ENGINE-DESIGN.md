...
| **P1（商业壁垒）** | AI 翻译 | 全书 AI 翻译（保留排版） | Manifest + 章节并发 + SQLite 缓存 + 失败救援 + 成品 QA | ✅ 已实现 |
...
| **P2（AI深水区）** | 图片文字翻译 | OCR 识别并翻译图片像素内文字 | 覆盖扫描页、图表标注和嵌字说明；失败时安全回退原图 | 🔲 TODO |

---

## 二、已实现的标准编译 Pipeline

> 下图描述普通转换以及快速翻译的“非 LLM 预处理”阶段。EPUB AI 翻译默认由 `job_runner -> fast_translation_runner` 编排，不是在 `ExtremeCompiler` 内逐文件调用 `SemanticsTranslator`；完整主链路见 [当前架构](ARCHITECTURE-DIAGRAM.md)。`enable_translation` 分支保留用于非 EPUB 翻译回退和兼容路径。
...
### 4.4 图片像素文字 OCR 翻译（TODO）

**现状边界**：主翻译链路已经翻译 XHTML 中的 `caption`、`figcaption`、`legend` 等文本型图片说明，但不会识别或改写图片像素内部的文字。

**拟议方案**：增加可选 OCR 阶段，输出文字、位置框、方向和置信度；复用全书术语表完成翻译，再通过可访问性文本、覆盖层或受控图片重绘回写。低置信度、排版溢出或重绘失败时必须回退原图，并在任务报告中单独记录 OCR 候选数、成功数、低置信度数和失败数。

详细接入位置、质量门禁与验收条件见 [当前架构](ARCHITECTURE-DIAGRAM.md#8-todo图片像素文字-ocr-翻译)。
