...
**解决方案**：`asyncio` + `AdaptiveConcurrencyLimiter` 控制单本书的模型请求并发。标准模式默认上限 6，高质量/文学模式默认上限 4；请求失败时自动降并发，连续成功后逐步恢复，并继续配合指数退避处理 429 与上游抖动。JSON 批量也会按质量档位动态扩容，失败时递归拆小。

### 1.4 中断重翻成本

翻译中途失败若从头重翻，浪费大量 Token 费用。

**解决方案**：SQLite 本地缓存（`translation_cache.db`）。缓存命名空间同时包含目标语言、提示词版本、质量档位、模型、温度、术语表哈希、上下文哈希和文学风格档案哈希；标准模式可复用兼容缓存，高质量与文学模式默认 `verified`，只读取完全同配置的精确缓存。

### 1.5 失败时已转换内容的存储与复用

**现状**：

- **跨任务/重试复用**：标准模式按版本化命名空间复用同配置的译文，兼容重译可在同一语言/质量/模型/温度族内读取最近缓存。高质量与文学模式默认只读取完全同配置的精确缓存，防止 Flash 旧译文或不同风格档案的译文直接进入高级质量链路；需要强制重跑时仍可显式选择 `fresh`。
- **同任务续跑（Resume）**：当前 EPUB 快速翻译主链路使用持久化书级准备结果和逐块检查点。键绑定订单、原书哈希、Manifest、模型、质量档及已确认设定；续跑只复用与当前设置相容且再次通过 QA 的结果。已完成的术语/画像准备及最终审校也可复用，不仅依赖普通译文缓存。旧的 `ExtremeCompiler` 同步翻译分支仍以段落缓存复用为主，不能混称为相同的恢复机制。

**建议**：

- 产品/文案应说明：相同订单和相容设置下，通过质检的已完成内容可从检查点恢复；修改设定、选择重新翻译、旧结果未通过当前 QA 或请求在保存前中断时，仍可能产生新请求。不能承诺任意重新上传都不重复计费。
- `reuse` / `verified` 可跨 attempt 恢复；`fresh` 只在同一 attempt 内恢复。中断前记录的请求预算不因 Worker 重启清零，预处理、打包与最终校验仍需执行。实现及离线验证边界见 [翻译卡点修复与验证边界](TRANSLATION-PERFORMANCE-2026-09-18.md)；真实服务故障恢复仍需单独在线验收。
...
flowchart TD
    ...
    D1 --> E["全书术语表 + 书名翻译"]
    D2 --> E
    E --> F["标准：Flash/0.3/reuse<br/>高质量：Flash/0.2/verified + 选择性审校<br/>文学：Flash/0.2/verified + 风格档案<br/>可显式选择 Pro，坏段落补译可升级"]
    F --> G{"SQLite 缓存命中?"}
    G -->|"是"| H["使用缓存译文"]
    ...
    J --> J2{"质量模式"}
    J2 -->|"standard"| K
    J2 -->|"high"| J3["风险规则 + 稳定抽样<br/>选择性 Pro 语义校对"]
    J2 -->|"literary"| J4["连续章节润色<br/>对照原文语义回查"]
    J3 --> K
    J4 --> J5["HTML/数字/术语/长度安全门禁"]
    J5 --> K
    H --> K["按 locator 回写章节"]
    K --> L["失败 chunk 补译队列"]
    L --> M{"预打包失败率门禁"}
...
✅ 已实现。用户术语表与清理后的自动/全局术语表合并；每段只注入实际出现的最长匹配术语，并通过版本化缓存命名空间隔离译文。
...
✅ 高质量与文学模式已实现上下文记忆。每个 chunk 携带书名、章节文件、上一段和下一段的截断文本作为只读 `context`，用于消歧、指代和语气衔接；Prompt 明确禁止把上下文补写进当前译文。标准模式不增加这部分 Token。

### 7.4 选择性审校与文学模式

✅ 高质量模式不再机械二次审校全书。它先用确定性规则识别长段、标题、复杂 HTML、数字/逻辑词/术语异常和疑似原文，再对其余段落做稳定抽样，只将命中的内容交给所选主模型复核。默认首选 Flash，显式 Pro 选择仍有效，失败补译可升级。

✅ 文学模式在首译前从全书多个位置抽取代表段落，生成一次紧凑的书级风格档案；首译后按章节连续上下文润色，再对照原文做语义回查。润色或回查结果若改变 HTML 结构、数字、强制术语，或长度变化超出安全范围，会被拒绝并保留上一版安全译文。
