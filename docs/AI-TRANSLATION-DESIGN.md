...
flowchart TD
    ...
    D1 --> E["全书术语表 + 书名翻译"]
    D2 --> E
    E --> F["标准：Flash/0.3/reuse<br/>高质量：Pro/0.2/verified + 选择性审校<br/>文学：Pro/0.2/verified + 风格档案"]
    F --> G{"SQLite 缓存命中?"}
    G -->|"是"| H["使用缓存译文"]
...
✅ 高质量模式不再机械二次审校全书。它先用确定性规则识别长段、标题、复杂 HTML、数字/逻辑词/术语异常和疑似原文，再对其余段落做稳定抽样，只将命中的内容交给 Pro 复核。
