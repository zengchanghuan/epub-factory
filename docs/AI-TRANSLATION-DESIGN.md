...
**解决方案**：SQLite 本地缓存（`translation_cache.db`）。缓存命名空间同时包含目标语言、提示词版本、质量档位、模型、温度、术语表哈希和上下文哈希；标准模式可复用兼容缓存，高质量模式默认 `fresh`，避免旧译文污染质量重译。

### 1.5 失败时已转换内容的存储与复用

**现状**：

- **跨任务/重试复用**：标准模式按版本化命名空间复用同配置的译文，兼容重译可在同一语言/质量/模型/温度族内读取最近缓存。高质量重译默认跳过所有缓存，但仍写入独立的新缓存，防止 Flash 旧译文直接冒充 Pro 校对结果。
...
flowchart TD
    ...
    D1 --> E["全书术语表 + 书名翻译"]
    D2 --> E
    E --> F["标准：Flash/0.3/reuse<br/>高质量：Pro/0.2/fresh + 章节上下文"]
    F --> G{"SQLite 缓存命中?"}
    G -->|"是"| H["使用缓存译文"]
    G -->|"否"| I["LLM 调用 + HTML/漏译质检"]
    I --> J["质量重试 / 模型升级 / 文本节点救援"]
    J --> J2{"高质量模式?"}
    J2 -->|"是"| J3["Pro 二次语义校对<br/>结构不安全则保留首译"]
    J2 -->|"否"| K
    J3 --> K
    H --> K["按 locator 回写章节"]
    K --> L["失败 chunk 补译队列"]
...
✅ 高质量模式已实现。每个 chunk 携带书名、章节文件、上一段和下一段的截断文本作为只读 `context`，用于消歧、指代和语气衔接；Prompt 明确禁止把上下文补写进当前译文。标准模式不增加这部分 Token。
