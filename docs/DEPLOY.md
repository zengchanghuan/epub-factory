...
2026-09-18 非 AI 格式转换默认价为 **1.99 元/本**，批量转换按本数相乘；独立 EPUB 格式修复默认价为 **0.99 元/本**。部署在完成 `.env`/数据库备份后，将 `CONVERSION_PRICE_CNY` 的未设置/空值或精确旧值 `5.99` 改为 `1.99`，将 `REPAIR_PRICE_CNY` 的未设置/空值、精确旧值 `5.99` 或先前默认值 `1.99` 改为 `0.99`；其他显式自定义价和测试价保留。AI 翻译最低价、翻译固定价和精校阶梯价均不修改。已有转换/翻译/批量订单继续按下单时持久化的 `expected_amount` 验款，独立修复旧单继续按保存的冻结报价和支付金额处理，均不重写历史金额。
