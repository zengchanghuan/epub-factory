...
用户只需要理解两件事：**免费体验**和**单本付费**。AI 精校作为高级选项在勾选时提示，不放进主价格表，避免用户选择过载。

| 套餐 | 价格 | 限制 | 内容 | 支付通道 |
|---|---|---|---|---|
| **免费体验** | **¥0** | **每月 1 本，单文件 ≤ 20MB** | 基础 CSS 清洗 + 繁简标准转换（含两岸词典 + 专有名词） | - |
| **单本处理** | **¥0.99/本** | 单文件 ≤ 60MB | 非 AI 格式转换、排版优化、TOC 修复、设备优化、繁简标准转换 | 支付宝 |

基础转换默认 **¥0.99/本**，独立 EPUB 格式修复默认 **¥2.99/本**，分别配置。AI 精校不包含在基础转换价中，仍按正文有效字数阶梯计价，结账时单独列示后与基础转换合并支付。AI 翻译采用下述字符阶梯价；旧订单仍按下单时冻结的原价支付和验款，不因新价格重新定价。

### 5.2 AI 翻译阶梯价

标准 Flash 模式最低 **¥3.99**：前 30 万字符按 ¥0.05/千字符，30-100 万字符的超出部分按 ¥0.035/千字符，100 万字符以上的超出部分按 ¥0.025/千字符。高质量模式乘 1.5，文学模式乘 2；选择 DeepSeek Pro 时再乘 3.4。缓存命中仍按未命中字符报价，但不会低于对应档位最低价。支付前展示最终金额，已创建订单保留原冻结价格。

> 暂不在主价格页展示 Pro / Lifetime / 海外美元价格。先用「免费体验 → 单本付费」验证核心转化，后续有稳定数据后再设计会员。

### 5.3 高级选项：繁简「AI 精校用词」

普通繁简转换是字形映射，会产生「軟體 → 软体」这种「字对了、词不对」的结果。
对用户展示时只保留两个选择：

| 选项 | 价格 | 处理范围 | 适用场景 |
|---|---|---|---|
| **标准转换（默认）** | 免费额度内或随单本处理附带 | OpenCC + 自维护两岸词典 + 专有名词词典 | 大多数用户 |
| **AI 精校** | **¥3.99 起，按正文有效字数阶梯计价** | DeepSeek 处理「机车/窝心/班房/土豆」等上下文歧义词 | 文学小说、口语化文本 |

AI 精校在用户上传并完成文本解析后再报价，按**正文有效字数**计费（去除目录、导航、样式、空白与重复元数据）：
...
> 详细设计见 `docs/CROSS-STRAIT-LEXICON-DESIGN.md`。
> 精校模式 Token 不封顶；实际价格在支付前展示并确认，DeepSeek 余额不足或异常时**自动全额退款**。

### 5.4 商业原则

- **主价格只放 2 档**：免费体验 / 单本处理，降低决策成本。
- **免费额度收紧**：每月 1 本、≤20MB，足够试用，不适合批量白嫖。
- **AI 能力不承诺固定价或无限**：DeepSeek 精校按正文长度阶梯计价，不进入买断或无限会员。
- **国内优先人民币定价**：当前支付以支付宝为主，美元价格只保留在海外化阶段重新设计。
